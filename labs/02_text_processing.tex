...
\begin{quote}
    ...
    \begin{center}
    \begin{equation*}
	BIM(q,d) = \sum _{i=1}^{|q|} \log \left( \frac{N}{n_i} \right)
	\end{equation*}
    \end{center}
    
    \begin{center}
    \begin{equation*}
{ {\text{BM25}}(q,d)=\sum _{i=1}^{|q|}{\text{IDF}}(q_{i})\cdot {\frac {f(q_{i},d)\cdot (k_{1}+1)}{f(q_{i},d)+k_{1}\cdot \left(1-b+b\cdot {\frac {|d|}{\text{avgdl}}}\right)}},} \text{~~with~~} \mathrm{IDF}(t) = \log \left( \frac{|D|}{|\{d' \in D: t \in d'\}|} \right)
    \end{equation*}
    \end{center}
    
    \begin{center}
    \begin{equation*}
	LM(q,d) = P(\{t_1,\ldots,t_{|q|}\}|d) = \prod_{1<=i<=|q|} P(t_i|d) = \prod_{1<=i<=|q|} \frac{f{t,d}}{|d|}
	\end{equation*}
    \end{center}
\end{quote}
