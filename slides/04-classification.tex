...
\begin{frame}
    ...
    \begin{itemize}
    \item In simple cases, we can easily find one (or more) solution(s)
        \begin{itemize}
...
\begin{frame}
  ...
  \begin{block}{}
  Leverage supervised machine learning together with a method for representing textual information (e.g., the vector space model with TF-IDF weights)
  \end{block}
...
\begin{frame}
  ...
  \item Generative Bayesian classifiers (e.g., na{\"i}ve Bayes)
    ...
    \item Discover the class distribution most likely to have generated a test
      document
    \end{itemize}
  \item Linear discriminative classifiers (e.g., the perceptron, or support vector machines):
    \begin{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Similar documents are expected to be assigned the same class label
    \begin{itemize}
...
\begin{frame}
  ...
  \item Advantages:
        ...
        \item Collection updates trivial
...
\begin{frame}
  ...
  \item To reduce space requirements and speed up classification
    \begin{itemize}
    \item Find clusters in the data \emph{(clustering will be covered in the next lecture)}
    \item Store only a few statistical parameters per cluster
    \item Compare with documents in only the most promising clusters
    \end{itemize}
  \item However...
    \begin{itemize}
...
\begin{frame} \frametitle{Learning the Document Distribution}
  ...
  \item $\Theta$ is estimated based on two factors:
    \begin{enumerate}
...
%     \begin{block}{}
%     The previous slides introduced a simple probabilistic model for {\it ad-hoc} document retrieval, based on language modeling
%     \end{block}
