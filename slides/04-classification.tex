...
\begin{center}
$P(y=0|x) = \frac{1}{1 + e^{-(w_0 + \sum_iw_ix_i)}}$ ~~and~~ $P(y=1|x) = 1 - P(y=0|x) = \frac{e^{-(w_0 + \sum_iw_ix_i)}}{1 + e^{-(w_0 + \sum_iw_ix_i)}}$
\end{center}
...
\begin{center}
$\log(e^{-(w_0 + \sum_iw_ix_i)}) − \log(1+e^{-(w_0 + \sum_iw_ix_i)}) − \log(1) + \log(1+e^{-(w_0 + \sum_iw_ix_i)}) > 0$
\end{center}
Thus, we see that the decision boundary is given by the plane $w_0 + \sum_i w_i \dot x_i$ (similarly to the Perceptron or SVM classifiers).
\end{frame}
...
\begin{frame}
    ...
    \end{itemize}
    \item Addressed latter in the course
    \end{itemize}
\end{frame}
