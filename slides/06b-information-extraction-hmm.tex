...
\begin{frame} \frametitle{The Forward-Backward Procedure}
...
\begin{frame} \frametitle{Computing The Forward-Backward Procedure}
...
\begin{frame} \frametitle{Unsupervised Learning of HMMs}
  ...
  \item We present one method: \emph{Segmental K-means}
  ...
  \item Other methods also exist (e.g., the \emph{Baum-Welch} method)
  \end{itemize}
...
\begin{frame}
    ...
    \begin{displaymath}
        P(y|x) = \frac{1}{Z(x)}\exp\left\{\sum_{k=1}^K\lambda_kf_k(y_t, y_{t-1},
          x_t)\right\}
    \end{displaymath}
\end{frame}
