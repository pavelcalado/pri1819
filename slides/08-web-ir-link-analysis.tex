...
\begin{frame}
  ...
  \item $L$ is \emph{aperiodic}
      ...
      \item the reverse---periodic---means that a node is visited always after
          ...
          % https://math.stackexchange.com/questions/112151/what-values-makes-this-markov-chain-aperiodic#112180
...
\begin{frame}
    \frametitle{Dangling Links}
    \begin{itemize}
    \item \emph{Dangling Links} are links to pages with no outgoing links
    \item These pages receive PageRank but do not share it with other pages
        \begin{itemize}
        \item In a sense, they are \textit{wasting} their PageRank
        \end{itemize}
    \item Ways to deal with it:
        \begin{itemize}
        \item Remove the links from the computation and add them in final
            iteration (original proposal)
        \item Consider a virtual link from the linked page to all pages (including itslef)
        \end{itemize}
    \end{itemize}
\end{frame}

% original: http://ilpubs.stanford.edu:8090/422/1/1999-66.pdf
% also: https://pr.efactory.de/e-outbound-links.shtml
...
\item repeat some important terms
 \begin{quote}the picture \emph{quality} of the camera
   is of amazing \emph{quality}\end{quote}

\item dumping of many unrelated terms 
\begin{quote}\end{quote} 
Ed Sheeran Dua Lipa Sam Smith Eminem Pink Camila Cabello Paloma Faith
\end{itemize}
\end{block}
