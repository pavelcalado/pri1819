...
\item Simple solution: calculate score for each field and combine them linearly
\end{itemize}
    \begin{displaymath}
          sim(d_j,q) =
          \sum_{f \in F} \alpha^f \times sim(d_j^f,q)
    \end{displaymath}
\begin{block}{Problems of linear combination}
\begin{itemize}
\item If corpus statistics are computed per field, IDF can vary highly in different fields (e.g. stopwords scoring highly in a title field)
...
\begin{block}{}
    \begin{displaymath}
          TF_{i,j} = \sum_{f \in F} \alpha^f \times \frac{f_{i,j} \times (k_1 + 1)}
                                  {f_{i,j} + k_1 \times \left( 1-b^f+b^f\frac{|d_j^f|}{avgdl^f} \right)}
    \end{displaymath}
    \begin{displaymath}
        IDF_i = \log\frac{N - n_i + 0.5}{n_i + 0.5}
    \end{displaymath}
